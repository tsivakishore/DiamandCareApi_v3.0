...
{
    public class RegisterByInstitutionRepository
    {
        ...
        public RegisterByInstitutionRepository()
        {
        ...
        public async Task<Tuple<bool, string>> RegisterUserByInstitution(User userModel)
        {
            ...
            try
            {
                ...
                if (identityUser.Succeeded)
                {
                    ...
                    using (SqlConnection cxn = new SqlConnection(_dcDb))
                    {
                        ...
                        parameters.Add("@CreatedBy", userModel.UserID, DbType.Int32);
                        parameters.Add("@CreatedDate", DateTime.Now, DbType.DateTime);

                        isUserInserted = await cxn.ExecuteScalarAsync<int>("dbo.InsertOrUpdate_UserDetailsByEduInstitution", parameters, commandType: CommandType.StoredProcedure);
                        cxn.Close();
        ...
        public async Task<Tuple<bool, string, List<UsersByInstitutionViewModel>>> GetUsersByInstitution(int UserID)
        {
            ...
            List<UsersByInstitutionViewModel> lstUsers = new List<UsersByInstitutionViewModel>();
            ...
            try
            {
                ...
                using (SqlConnection cxn = new SqlConnection(_dcDb))
                {
                    ...
                    using (var multi = await cxn.QueryMultipleAsync("[dbo].[Select_UsersByInstitution]", parameters, commandType: CommandType.StoredProcedure))
                    {
                        lstUsers = multi.Read<UsersByInstitutionViewModel>().ToList();
                    }
                    cxn.Close();
                }

                if (lstUsers != null && lstUsers.Count > 0)
                    result = Tuple.Create(true, "", lstUsers);
                else
                    result = Tuple.Create(false, "", lstUsers);
            }
            catch (Exception ex)
            {
                ErrorLog.Write(ex);
